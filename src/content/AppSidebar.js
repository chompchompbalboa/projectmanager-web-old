...
// Component
//-----------------------------------------------------------------------------
const AppSidebar = ({ activeContent, activeContentChoices, changeActiveContent }) => {
  return (
    <Container>
      {activeContentChoices.map((activeContentChoice, index) => {
        // Convert the name from uppercase to first letter uppercase
        const activeContentChoiceName = activeContentChoice.slice(0, 1) + activeContentChoice.slice(1, activeContentChoice.length).toLowerCase()
        return (
          <ActiveContentChoice
            key={index}
