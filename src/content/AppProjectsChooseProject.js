...
export default class AppProjectsChooseProject extends Component {
  ...
  changeDropdownInputValue = (nextDropdownInputValue) => {
    const { 
      nextProjects, 
      nextIsOnlyOneProjectVisible 
    } = this.filterProjects(nextDropdownInputValue)
    this.setState({
  ...
  filterProjects = (searchValue) => {
    ...
    const isOnlyOneProjectVisible = filteredProjects.length === 0 ? false : filteredProjects.length === 1
    filteredProjects = filteredProjects.length === 0 ? [{ name: "No results found", uniqueId: null }] : filteredProjects
    return { 
    ...
    return (
      <Container>
        <CurrentProjectContainer>
          ...
          <CurrentProjectSeparator>
            /
          </CurrentProjectSeparator>
          <CurrentProjectUniqueId>
            {activeProject.uniqueId}
          </CurrentProjectUniqueId>
        </CurrentProjectContainer>
        <DropdownContainer
...
const CurrentProjectSeparator = styled.div`
  margin: 0 0.5vw;
  font-weight: bold;
  font-size: 22px;
`

const CurrentProjectUniqueId = styled.div`
  font-size: 16px;
  &:hover {
    color: ${ colors.PRIMARY };
  }
...
const DropdownProjectContainer = styled.div`
  ...
  align-items: center;
  color: ${ props => props.isOnlyOneProjectVisible ? colors.PRIMARY : 'inherit' };
  border-right: ${ props => props.isOnlyOneProjectVisible ? '3px solid ' + colors.PRIMARY : 'none' };
  &:hover {
    color: ${ colors.PRIMARY };
...
const DropdownProjectUniqueId = styled.div`
`